...
def Player_fight(player1: dict, player2: dict, type_in, bot_id):
    ...
    if user2_buff_date.get_user_sec_buff_data() != None:
        ...
        user2_skill_date = {'shuxing': '无'}


    # ori_atk = user1_buff_date['atk']
    # player1_sh = get_turnatk(player1)
    atkbl1 = 1
    atkbl2 = 1

    # 获取玩家的神通属性数据
    if user1_skill_date['shuxing'] == '金':
        if user2_skill_date['shuxing'] == '木':
            atkbl1 = 1.4
        if user2_skill_date['shuxing'] == '火':
            atkbl1 = 0.6
    if user1_skill_date['shuxing'] == '木':
        if user2_skill_date['shuxing'] == '土':
            atkbl1 = 1.4
        if user2_skill_date['shuxing'] == '金':
            atkbl1 = 0.6
    if user1_skill_date['shuxing'] == '水':
        if user2_skill_date['shuxing'] == '火':
            atkbl1 = 1.4
        if user2_skill_date['shuxing'] == '土':
            atkbl1 = 0.6
    if user1_skill_date['shuxing'] == '火':
        if user2_skill_date['shuxing'] == '金':
            atkbl1 = 1.4
        if user2_skill_date['shuxing'] == '水':
            atkbl1 = 0.6
    if user1_skill_date['shuxing'] == '土':
        if user2_skill_date['shuxing'] == '水':
            atkbl1 = 1.4
        if user2_skill_date['shuxing'] == '木':
            atkbl1 = 0.6
    if user1_skill_date['shuxing'] == "光":
        if user2_skill_date['shuxing'] == '暗':
            atkbl1 = 1.4
    if user1_skill_date['shuxing'] == "暗":
        if user2_skill_date['shuxing'] == '光':
            atkbl1 = 1.4
    ...
    while True:
        ...
        if player1_skil_open:  # 是否开启技能
            if user1_turn_skip:  # 无需跳过回合
                play_list.append(get_msg_dict(player1, player1_init_hp, f"☆------{player1['道号']}的回合,属性为{user1_skill_date['shuxing']},对方属性为{user2_skill_date['shuxing']},造成{atkbl1}倍伤害------☆"))
                ...
                if player1_turn_cost == 0:  # 没有持续性技能生效
                    ...
                    if isEnableUserSikll(player1, user1_hp_cost, user1_mp_cost, player1_turn_cost,
                        ...
                        user1_skill_shh = user1_skill_sh * atkbl1 # 神通伤害 * 属性伤害倍率
                        ...
                        if user1_skill_type == 1:  # 直接伤害类技能
                            ...
                            player2['气血'] = player2['气血'] - int(user1_skill_shh * player2_js)  # 玩家1的伤害 * 玩家2的减伤
                            ...
                            if user1_buff_type == 1:  # 攻击类buff
                                ...
                                player1 = calculate_skill_cost(player1, user1_hp_cost, user1_mp_cost)
                                play_list.append(get_msg_dict(player1, player1_init_hp, skill_msg))
                                play_list.append(get_msg_dict(player1, player1_init_hp, msg1.format(player1['道号'], player1_shh)))
                                player2['气血'] = player2['气血'] - int(player1_shh * player2_js)  # 玩家1的伤害 * 玩家2的减伤
                                play_list.append(get_msg_dict(player1, player1_init_hp, f"{player2['道号']}剩余血量{player2['气血']}"))
                    ...
                    if user1_skill_type == 2:  # 持续性伤害技能
                        ...
                        skill_msg = get_persistent_skill_msg(player1['道号'], user1_skill_date['name'], user1_skill_shh,
                                                            player1_turn_cost)
                        play_list.append(get_msg_dict(player1, player1_init_hp, skill_msg))
                        isCrit, player1_sh = get_turnatk(player1)  # 判定是否暴击
                        player1_shh = player1_sh * atkbl1
                        ...
                        player2['气血'] = player2['气血'] - int((user1_skill_shh + player1_shh) * player2_js)
                        play_list.append(get_msg_dict(player1, player1_init_hp, f"{player2['道号']}剩余血量{player2['气血']}"))
                        ...
                        if user1_buff_type == 1:  # 攻击类buff
                            isCrit, player1_sh = get_turnatk(player1, user1_skill_sh)  # 判定是否暴击
                            user1_skill_shh = player1_sh * atkbl1
                            if isCrit:
                                msg1 = "{}发起会心一击，造成了{}伤害\n"
                            else:
                                msg1 = "{}发起攻击，造成了{}伤害\n"
            ...
            if user1_turn_skip:
                ...
                if isCrit:
                    ...
                    msg1 = "{}发起攻击，造成了{}伤害\n"
                play_list.append(get_msg_dict(player1, player1_init_hp, msg1.format(player1['道号'], player1_shh)))
                player2['气血'] = player2['气血'] - player1_shh
                play_list.append(get_msg_dict(player1, player1_init_hp, f"{player2['道号']}剩余血量{player2['气血']}"))
        ...
        if player2_skil_open:  # 有技能
            if user2_turn_skip:  # 玩家2无需跳过回合
                play_list.append(get_msg_dict(player2, player2_init_hp, f"☆------{player2['道号']}的回合,属性为{user2_skill_date['shuxing']},对方属性为{user1_skill_date['shuxing']},造成{atkbl2}倍伤害------☆"))
                ...
                if player2_turn_cost == 0:  # 没有持续性技能生效
                    ...
                    if isEnableUserSikll(player2, user2_hp_cost, user2_mp_cost, player2_turn_cost,
                        ...
                        user2_skill_shh = user2_skill_sh * atkbl2
                        if user2_skill_type == 1:  # 直接伤害类技能
                            play_list.append(get_msg_dict(player2, player2_init_hp, skill_msg))
                            player1['气血'] = player1['气血'] - int(user2_skill_shh * player1_js)  # 玩家2的伤害 * 玩家1的减伤
                            ...
                            if user2_buff_type == 1:  # 攻击类buff
                                ...
                                player1['气血'] = player1['气血'] - int(player2_shh * player1_js)
                                play_list.append(get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{player1['气血']}"))
                                player2 = calculate_skill_cost(player2, user2_hp_cost, user2_mp_cost)
                                ...
                                if isCrit:
                                    ...
                                    msg2 = "{}发起攻击，造成了{}伤害\n"
                                play_list.append(get_msg_dict(player2, player2_init_hp, skill_msg))
                                play_list.append(get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], player2_shh)))
                                player1['气血'] = player1['气血'] - int(player2_shh * player1_js)
                                play_list.append(get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{player1['气血']}"))
                        ...
                        if isCrit:
                            ...
                            msg2 = "{}发起攻击，造成了{}伤害\n"
                        play_list.append(get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], player2_shh)))
                        player1['气血'] = player1['气血'] - int(player2_shh * player1_js)  # 玩家2的伤害 * 玩家1的减伤
                        play_list.append(get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{player1['气血']}"))
                    ...
                    if user2_skill_type == 2:  # 持续性伤害技能
                        ...
                        play_list.append(get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], player2_shh)))
                        player1['气血'] = player1['气血'] - int((user2_skill_shh + player2_shh) * player1_js)
                        play_list.append(get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{player1['气血']}"))
                        ...
                        user2_buff_type = user2_skill_date['bufftype']
                        if user2_buff_type == 1:  # 攻击类buff
                            isCrit, player2_sh = get_turnatk(player2, user2_skill_shh)  # 判定是否暴击
                            player2_shh = player2_sh * atkbl2
                            if isCrit:
                                msg2 = "{}发起会心一击，造成了{}伤害\n"
                            else:
                            ...
                            play_list.append(get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], player2_shh)))
                            player1['气血'] = player1['气血'] - int(player2_shh * player1_js)  # 玩家1的伤害 * 玩家2的减伤
                            play_list.append(get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{player1['气血']}"))
            ...
            if user2_turn_skip:
                ...
                if isCrit:
                    ...
                    msg2 = "{}发起攻击，造成了{}伤害\n"
                play_list.append(get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], player2_shh)))
                player1['气血'] = player1['气血'] - player2_shh
                play_list.append(get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{player1['气血']}"))
...
# 金克木，木克土，土克水，水克火，火克金
    ...
    while True:
        ...
        if player1_skil_open:  # 是否开启技能
            if user1_turn_skip:  # 无需跳过回合
                turn_start_msg = f"☆------{player1['道号']}的回合,属性为{user1_skill_date['shuxing']},对方属性为{boss['shuxing']},造成{atkbl1}倍伤害------☆"
                ...
                if player1_turn_cost == 0:  # 没有持续性技能生效
                    ...
                    if isEnableUserSikll(player1, user1hpconst, user1mpcost, player1_turn_cost,
                        ...
                        user1_skill_sh = user1_skill_sh * atkbl1
            ...
            play_list.append(get_msg_dict(player1, player_init_hp, f"☆------{player1['道号']}的回合属性为{user1_skill_date['shuxing']},对方属性为{boss['shuxing']},造成{atkbl1}倍伤害------☆"))
        ...
        if player1_turn_cost < 0:  # 休息为负数，如果休息，则跳过回合，正常是0
            ...
            # 没有技能的derB
        if boss_turn_skip:
            play_list.append(get_boss_dict(boss, qx, f"☆------{boss['name']}的回合,属性为{boss['shuxing']},对方属性为{user1_skill_date['shuxing']},造成{atkbl2}倍伤害------☆", bot_id))
            # play_list.append(get_boss_dict(boss, qx, f"☆------{boss['name']}的回合------☆", bot_id))
            isCrit, boss_sh = get_turnatk(boss)  # 判定是否暴击
            boss_sh = boss_sh * atkbl2
            if isCrit:
                msg2 = "{}发起会心一击，造成了{}伤害\n"
            else:
                msg2 = "{}发起攻击，造成了{}伤害\n"
