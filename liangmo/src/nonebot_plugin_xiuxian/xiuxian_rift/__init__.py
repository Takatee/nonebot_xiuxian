...
from .riftmake import Rift, get_rift_type, get_story_type, NONEMSG, get_battle_type, get_dxsj_info, get_boss_battle_info, get_treasure_info,check_ylsf,TIME,get_ylsf_treasure_info
import random
...
create_rift = on_command("生成秘境", priority=5, permission= SUPERUSER)
...
ylsf = on_command("游历四方",aliases={"云游","红尘之旅"},priority=5)
ylsfjs = on_command("结算游历四方",aliases={"结算云游","结算红尘之旅"},priority=5)
__rift_help__ = f"""
秘境帮助信息:
指令：
...
6.秘境帮助：获取秘境帮助信息
7.云游，游历四方：红尘修炼，获得更多经验
8.结算云游，结算游历四方：结算云游所获得的东西
非指令：
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if group_id not in groups:
        ...
        if XiuConfig().img:
            ...
            await create_rift.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await explore_rift.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await complete_rift.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await break_rift.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
            await ylsf.finish(MessageSegment.image(pic))
        else:
            await ylsf.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    needtime = user_info.ylsf
    nowtime = datetime.now()
    user_message = sql_message.get_user_message(user_id)

    #判断修为，小于20968000提示不可以云游
    if user_message.exp < 20968000: #半步真仙
        msg = "道友的修为去游历非常危险哦，请先提升修为！"
        if XiuConfig().img:
            pic = await get_msg_pic(msg)
            await ylsfjs.finish(MessageSegment.image(pic))
        else:
            await ylsfjs.finish(f"@{event.sender.nickname}" + msg)

    else:       

        ylsf_cd = user_message.ylsf_cd
        if  ylsf_cd:
            time =OtherSet().date_diff(nowtime,ylsf_cd)
            ...
            if  time <= needtime:
                ...
                if XiuConfig().img:
                    ...
                    await ylsfjs.finish(f"@{event.sender.nickname}" + msg)
            if needtime > 0:
                msg = "道友目前的云游奖励还未结算，请输入“结算云游”或者“结算游历四方”收取奖励"
                if XiuConfig().img:
                    pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
                    await ylsfjs.finish(MessageSegment.image(pic))
                else:
                    await ylsfjs.finish(f"@{event.sender.nickname}" + msg)
        needtime = random.randint(3,6) * 3600
        sql_message.ylsf_need_time(needtime,user_id)
        sql_message.ylsf(user_id)
        msg = "道友下山历练，游历凡尘，大约{}小时后可终止游历".format(needtime // 3600)
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
            await ylsf.finish(MessageSegment.image(pic))
        else:
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if needtime == 0:
        msg = "道友目前还未进行云游哦~请输入“云游”或者“游历四方”进行云游哦~"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
            await ylsfjs.finish(MessageSegment.image(pic))
        else:
    ...
    needtime = 0
    sql_message.ylsf_need_time(needtime,user_id)
    msg = get_ylsf_treasure_info(user_info)
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
        await ylsfjs.finish(MessageSegment.image(pic))
    else:
    ...
    if mode == '开启':
        if is_in_group:
            ...
            if XiuConfig().img:
                ...
                await set_group_rift.finish(f"@{event.sender.nickname}" + msg)
        else:
