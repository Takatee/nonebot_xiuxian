...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_elixir_room_make.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_elixir_get.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_buff_info.finish(f"@{event.sender.nickname}" + msg)
    sect_id = user_info.sect_id
    if sect_id:
        sect_info = sql_message.get_sect_info(sect_id)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_mainbuff_learn.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_mainbuff_get.finish(f"@{event.sender.nickname}" + msg)
    sect_id = user_info.sect_id
    if sect_id:
        sect_position = user_info.sect_position
        owner_idx = [k for k, v in jsondata.sect_config_data().items() if v.get("title", "") == "宗主"]
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_secbuff_get.finish(f"@{event.sender.nickname}" + msg)
    sect_id = user_info.sect_id
    if sect_id:
        sect_position = user_info.sect_position
        owner_idx = [k for k, v in jsondata.sect_config_data().items() if v.get("title", "") == "宗主"]
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_secbuff_learn.finish(f"@{event.sender.nickname}" + msg)
    msg = args.extract_plain_text().strip()
    sect_id = user_info.sect_id
    if sect_id:
        sect_position = user_info.sect_position
    ...
    if sect_id:
        ...
        if useratkpractice == 25:
            ...
            if XiuConfig().img:
                ...
                await upatkpractice.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_task_refresh.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await sect_users.finish(f"@{event.sender.nickname}" + msg)
    ...
    if sect_id:
        ...
        if user_now_num >= config["每日宗门任务次上限"]:
            ...
            if XiuConfig().img:
                ...
                await sect_task.finish(f"@{event.sender.nickname}" + msg)
    ...
    if sect_id:
        if not isUserTask(user_id):
            ...
            if XiuConfig().img:
                ...
                await sect_task_complete.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot,event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not user_message:
        ...
        if XiuConfig().img:
            ...
            await sect_owner_change.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if XiuConfig().img:
        ...
        await create_sect.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot,event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not user_message:
        ...
        if XiuConfig().img:
            ...
            await sect_kick_out.finish(f"@{event.sender.nickname}" + msg)
    if not user_message.sect_id:
        msg = "道友还未加入一方宗门。"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot,event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not user_message:
        ...
        if XiuConfig().img:
            ...
            await sect_out.finish(f"@{event.sender.nickname}" + msg)
    if not user_message.sect_id:
        msg = "道友还未加入一方宗门。"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot,event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not user_message:
        ...
        if XiuConfig().img:
            ...
            await sect_donate.finish(f"@{event.sender.nickname}" + msg)
    if not user_message.sect_id:
        msg = "道友还未加入一方宗门。"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if user_message.sect_position > idx_position:
        ...
        if XiuConfig().img:
            ...
            await sect_position_update.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if XiuConfig().img:
        ...
        await join_sect.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if XiuConfig().img:
        ...
        await my_sect.finish(f"@{event.sender.nickname}" + msg)
