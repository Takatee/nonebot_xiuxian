...
UserDate = namedtuple("UserDate",
                      ["id", "user_id", "stone", "root", "root_type", "level", "power", "create_time", "is_sign", "exp",
                       ...
                       "sect_task", "sect_contribution", "sect_elixir_get", "blessed_spot_flag", "blessed_spot_name","hf_cd","ylsf_cd","ylsf"])
...
class XiuxianDateManage:
    ...
    def _check_data(self):
      ...
      "hf_cd" integer DEFAULT NULL,
      "ylsf_cd" integer DEFAULT NULL,
      "ylsf" integer DEFAULT NULL
    );""")
            elif i == "user_cd":
                try:
                    c.execute(f"select count(1) from {i}")
  ...
  "fabao_weapon" integer DEFAULT 0
    ...
    def get_root_power_rate(self, name):
        """获取灵根战力倍率"""
        data = jsondata.root_data()
        return data[name]['type_power_rate']
    ...
    def update_power2(self, user_id):
        """更新战力"""
        UserMessage = self.get_user_message(user_id)
        cur = self.conn.cursor()
        level = jsondata.level_data()
        root = jsondata.root_data()
        sql = f"UPDATE user_xiuxian SET power=round(exp*?,0),atk=exp*?/10 WHERE user_id=?"
        cur.execute(sql, (root[UserMessage.root_type]["type_power_rate"],root[UserMessage.root_type]["type_power_rate"], user_id))
        self.conn.commit()
    ...
    def update_root(self, user_id, key):
        """更新灵根  1为废灵根，2为伪灵根，3为人灵根，4为地灵根，5为天灵根，6为光灵根，7为暗灵根"""
        cur = self.conn.cursor()
        print(user_id)
        print(key)
        ...
        if int(key) == 1:
            ...
            cur.execute(sql, ("五属性灵根", "废灵根", user_id))
            self.conn.commit()
        elif int(key) == 2:
            sql = f"UPDATE user_xiuxian SET root=?,root_type=? WHERE user_id=?"
            cur.execute(sql, ("金木水火四属性灵根", "伪灵根", user_id))
            self.conn.commit()
        elif int(key) == 3:
            sql = f"UPDATE user_xiuxian SET root=?,root_type=? WHERE user_id=?"
            cur.execute(sql, ("金木水三属性灵根", "人灵根", user_id))
            self.conn.commit()
        elif int(key) == 4:
            sql = f"UPDATE user_xiuxian SET root=?,root_type=? WHERE user_id=?"
            cur.execute(sql, ("金木双属性灵根", "地灵根", user_id))
            self.conn.commit()
        elif int(key) == 5:
            sql = f"UPDATE user_xiuxian SET root=?,root_type=? WHERE user_id=?"
            ...
            sql = f"UPDATE user_xiuxian SET root=?,root_type=? WHERE user_id=?"
            cur.execute(sql, ("光属性灵根", "光灵根", user_id))
            self.conn.commit()                        
        elif int(key) == 7:
            sql = f"UPDATE user_xiuxian SET root=?,root_type=? WHERE user_id=?"
            cur.execute(sql, ("暗属性灵根", "暗灵根", user_id))
            self.conn.commit()
    ...
    def update_give_allexp(self, price):
        """所有用户增加修为"""
        cur = self.conn.cursor()
        sql = f"UPDATE user_xiuxian SET exp=exp+?"
        cur.execute(sql, (price,))
        self.conn.commit()

    def update_del_all(self, price):
        """所有用户减少灵石"""
        cur = self.conn.cursor()
        sql = f"UPDATE user_xiuxian SET stone=stone-?"
        cur.execute(sql, (price,))
    ...
    def hfzt(self, user_id):
        """
        更新用户护法状态cd
        """
        now_time = datetime.datetime.now()
        # scheduled_time = datetime.datetime.now() + datetime.timedelta(minutes=int(the_time))
        sql = f"UPDATE user_xiuxian SET hf_cd=? where user_id=?"
        cur = self.conn.cursor()
        cur.execute(sql, (now_time, user_id))
        self.conn.commit()

    def ylsf(self, user_id):
        """
        更新用户游历四方状态cd
        """
        now_time = datetime.datetime.now()
        sql = f"UPDATE user_xiuxian SET ylsf_cd=? where user_id=?"
        cur = self.conn.cursor()
        cur.execute(sql, (now_time,user_id))
        self.conn.commit()
    
    def ylsf_need_time(self, needtime,user_id):
        """
        更新游历四方需要时间
        """
        sql = f"UPDATE user_xiuxian SET ylsf=? where user_id=?"
        cur = self.conn.cursor()
        cur.execute(sql, (needtime,user_id))
        self.conn.commit()
    ...
    def realm_top(self):
        ...
        ORDER BY CASE
        WHEN level = '天道主宰' THEN '48'
        WHEN level = '道仙境圆满' THEN '49'
        WHEN level = '道仙境中期' THEN '50'
        WHEN level = '道仙境初期' THEN '51'
        WHEN level = '道尊境圆满' THEN '52'
        WHEN level = '道尊境中期' THEN '53'
        WHEN level = '道尊境初期' THEN '54'
        WHEN level = '道祖境圆满' THEN '55'
        WHEN level = '道祖境中期' THEN '56'
        WHEN level = '道祖境初期' THEN '57'
        WHEN level = '大罗境圆满' THEN '58'
        WHEN level = '大罗境中期' THEN '59'
        WHEN level = '大罗境初期' THEN '60'
        WHEN level = '太乙境圆满' THEN '61'
        WHEN level = '太乙境中期' THEN '62'
        WHEN level = '太乙境初期' THEN '63'
        WHEN level = '金仙境圆满' THEN '64'
        WHEN level = '金仙境中期' THEN '65'
        WHEN level = '金仙境初期' THEN '66'
        WHEN level = '真仙境圆满' THEN '67'
        WHEN level = '真仙境中期' THEN '68'
        WHEN level = '真仙境初期' THEN '69'
        WHEN level = '半步真仙' THEN '70'
        WHEN level = '渡劫境圆满' THEN '71'
        WHEN level = '渡劫境中期' THEN '72'
        WHEN level = '渡劫境初期' THEN '73'
        WHEN level = '大乘境圆满' THEN '74'
        WHEN level = '大乘境中期' THEN '75'
        WHEN level = '大乘境初期' THEN '76'
        WHEN level = '合体境圆满' THEN '77'
        WHEN level = '合体境中期' THEN '78'
        WHEN level = '合体境初期' THEN '79'
        WHEN level = '炼虚境圆满' THEN '80'
        WHEN level = '炼虚境中期' THEN '81'
        WHEN level = '炼虚境初期' THEN '82'
        WHEN level = '化神境圆满' THEN '83'
        WHEN level = '化神境中期' THEN '84'
        WHEN level = '化神境初期' THEN '85'
        WHEN level = '元婴境圆满' THEN '86'
        WHEN level = '元婴境中期' THEN '87'
        WHEN level = '元婴境初期' THEN '88'
        WHEN level = '结丹境圆满' THEN '89'
        WHEN level = '结丹境中期' THEN '90'
        WHEN level = '结丹境初期' THEN '91'
        WHEN level = '筑基境圆满' THEN '92'
        WHEN level = '筑基境中期' THEN '93'
        WHEN level = '筑基境初期' THEN '94'
        WHEN level = '练气境圆满' THEN '95'
        WHEN level = '练气境中期' THEN '96'
        WHEN level = '练气境初期' THEN '97'
        WHEN level = '江湖好手' THEN '98'
    ...
    def update_levelrate_plus(self, user_id, rate, key):
        """加减突破成功率，1为加，2为减"""
        cur = self.conn.cursor()

        if key == 1:
            sql = f"UPDATE user_xiuxian SET level_up_rate=level_up_rate+? WHERE user_id=?"
            cur.execute(sql, (rate, user_id))
            self.conn.commit()
        elif key == 2:
            sql = f"UPDATE user_xiuxian SET level_up_rate=level_up_rate-? WHERE user_id=?"
    ...
    def update_user_hp(self,user_id):
        """重置用户状态信息"""
        sql = f"UPDATE user_xiuxian SET hp=exp/2,mp=exp,atk=power/10 where user_id=?"
        cur = self.conn.cursor()
        cur.execute(sql, (user_id,))
        self.conn.commit()

    def update_user_helf_exp(self,user_id):
        """重置用户,exp减半，stone减半"""
        sql = f"UPDATE user_xiuxian SET exp=exp/2,stone=stone/2 where user_id=?"
        cur = self.conn.cursor()
        cur.execute(sql, (user_id,))
        self.conn.commit()
    ...
    def restate(self, user_id=None):
        ...
        if user_id is None:
            sql = f"UPDATE user_xiuxian SET hp=exp/2,mp=exp,atk=power/10"
            cur = self.conn.cursor()
            cur.execute(sql,)
            self.conn.commit()
        else:
    ...
    def update_back_delete(self, user_id, goods_id, num):
        """
        丢弃物品
        """
        back = self.get_item_by_good_id_and_user_id(user_id, goods_id)
