...
create = on_command("生成世界boss", aliases={"生成世界Boss", "生成世界BOSS"}, priority=5, permission= SUPERUSER)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isInGroup:#不在配置表内
        ...
        if XiuConfig().img:
            ...
            await boss_delete.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isInGroup:#不在配置表内
        ...
        if XiuConfig().img:
            ...
            await boss_delete_all.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isInGroup:#不在配置表内
        ...
        if XiuConfig().img:
            ...
            await battle.finish(f"@{event.sender.nickname}" + msg)
    ...
    us_rank = int(user_rank-boss_rank)
    if us_rank >= 6:
        base_integral = 200
        base_boss_get_boss_xiuwei = 10000
    elif us_rank >= -6:
        base_integral = 100
        base_boss_get_boss_xiuwei = 3000
    elif us_rank < -6:
        base_integral = 100 + us_rank * 20
        base_boss_get_boss_xiuwei = 3000 + us_rank * 428
    elif us_rank <= -25:
        base_integral = 0
    elif us_rank <= -40:
        base_boss_get_boss_xiuwei = 0
    if victor == "Boss赢了":
        ...
        boss_integral = int(((boss_old_hp - boss_now_hp) / boss_all_hp) * base_integral)
        boss_get_true_xiuwei = int(((boss_old_hp - boss_now_hp)/ boss_all_hp)* base_boss_get_boss_xiuwei)
        if boss_integral < 10:#摸一下不给
            boss_integral = 0
        if boss_rank - user_rank >= 100:#超过太多不给
            boss_integral = 0
        ...
        sql_message.update_exp(user_id, boss_get_true_xiuwei)
        msg = f"道友不敌{bossinfo['name']}，重伤逃遁，临逃前收获灵石{get_stone}枚，{more_msg}获得世界积分：{boss_integral}点,修为：{boss_get_true_xiuwei}"
        battle_flag[group_id] = False
        try:
        ...
        #新增boss战斗积分点数
        boss_all_hp = bossinfo['总血量']#总血量
        boss_integral = int((boss_old_hp / boss_all_hp) * base_integral)
        boss_get_true_xiuwei = int((boss_old_hp / boss_all_hp)* base_boss_get_boss_xiuwei)
        if boss_integral < 10:#摸一下不给
            boss_integral = 0
        if boss_rank - user_rank >= 100:#超过太多不给
            boss_integral = 0
        ...
        save_user_boss_fight_info(user_id, user_boss_fight_info)
        sql_message.update_exp(user_id, boss_get_true_xiuwei)
        msg = f"恭喜道友击败{bossinfo['name']}，收获灵石{get_stone}枚，{more_msg}获得世界积分：{boss_integral}点，修为：{boss_get_true_xiuwei}"
        try:
            await send_forward_msg_list(bot, event, result)
        except ActionFailed:
            msg += "\nBoss战消息发送错误，可能被风控！"
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isInGroup:#不在配置表内
        ...
        if XiuConfig().img:
            ...
            await boss_info.finish(f"@{event.sender.nickname}" + msg)
...
携带灵石：{boss['stone']}
        ...
        i = 1
        for boss in bosss:
            bossmsgs += f"编号{i}、{boss['shuxing']}属性{boss['jj']}Boss:{boss['name']} \n"
            i += 1
        msg = bossmsgs
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
    ...
    if mode == '开启':
        if isInGroup:
            ...
            if XiuConfig().img:
                ...
                await set_group_boss.finish(f"@{event.sender.nickname}" + msg)
        else:
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await boss_integral_info.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await boss_integral_use.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    msg = args.extract_plain_text().strip()
