...
auction = {}
AUCTIONSLEEPTIME = 100#交友初始等待时间（秒）
...
reuse = on_command("回收",priority=5)
...
creat_auction = on_command("举行交友会", priority=5, permission= SUPERUSER)
...
10、背包帮助：获取背包帮助指令
11、丢弃xxx：丢弃背包内的物品
12、举行交友会：举行交友会，超管指令，注意：会在机器人所在的全部已开启此功能的群内通报交友消息
非指令：
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await buy.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await shop.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if args == []:
        ...
        if XiuConfig().img:
            ...
            await shop_added_by_admin.finish(f"@{event.sender.nickname}" + msg)
    goods_name = args[0]
    goods_id = -1
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await shop_added.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    args = args.extract_plain_text().split()
    if args == []:
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await shop_off.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    group_id = str(event.group_id)
    shop_data = get_shop_data(group_id)
    if shop_data[group_id] == {}:
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await mind_back.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
            await reuse.finish(MessageSegment.image(pic))
        else:
            await reuse.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    arg = args.extract_plain_text().strip()
    if re.match('(\D+)(\d+)',arg):
        string = re.match('(\D+)(\d+)',arg)
        goods_name = string.group(1)
        goods_need_num = int(string.group(2))
    ...
    if not in_flag:
        ...
        if XiuConfig().img:
            ...
            await reuse.finish(f"@{event.sender.nickname}" + msg)
    if goods_need_num > goods_num:
        msg = "道友的背包内没有足够数量的{}！".format(goods_name)
    reuse_ls = XiuConfig().reuse_ls * goods_need_num
    sql_message.update_back_delete(user_id, goods_id,goods_need_num)
    sql_message.update_ls(user_id, reuse_ls, 1)
    msg = "道友成功回收{}，并卖出{}枚灵石".format(goods_name,reuse_ls)
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
        await reuse.finish(MessageSegment.image(pic))
    else:
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if group_id not in groups:
        ...
        if XiuConfig().img:
            ...
            await creat_auction.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if group_id not in groups:
        ...
        if XiuConfig().img:
            ...
            await offer_auction.finish(f"@{event.sender.nickname}" + msg)
    ...
    for group_id in groups:
        try:
            ...
            error_msg = f"当前交友物品金额为：{auction['now_price']}！"
            continue
    ...
    if mode == '开启':
        if is_in_group:
            ...
            if XiuConfig().img:
                ...
                await set_auction.finish(f"@{event.sender.nickname}" + msg)
        else:
