...
import random
import re
# 导入back_util.py中的get_yaocai_msg
# from ..xiuxian_back import back_util
from ..xiuxian_back.back_util import get_yaocai_msg
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await mix_elixir_sqdj_up.finish(f"@{event.sender.nickname}" + msg)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await mix_elixir_dykh_up.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await mix_elixir_nyx_up.finish(f"@{event.sender.nickname}" + msg)
    msg = f"丹药耐药性的升级目前未开放！"
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}" + msg)
        await mix_elixir_nyx_up.finish(MessageSegment.image(pic))
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await yaocai_get.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    if int(user_info.blessed_spot_flag) == 0:
        msg = f"道友还没有洞天福地呢，请发送洞天福地购买吧~"
        if XiuConfig().img:
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await my_mix_elixir_info.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    mix_elixir_info = get_player_info(user_id, 'mix_elixir_info')
    l_msg = []
...
async def _mix_elixir(bot: Bot, event: GroupMessageEvent):
    ...
    if not isUser:
        if XiuConfig().img:
            ...
            await mix_elixir.finish(f"@{event.sender.nickname}" + msg)
    user_id = user_info.user_id
    ...
    if XiuConfig().img:
        ...
        await mix_elixir.send(f"@{event.sender.nickname}" + msg)
    ...
    if finall_mix_elixir_msg == {}:
        ...
        if XiuConfig().img:
            ...
            await mix_elixir.pause(prompt=f"@{event.sender.nickname}" + msg)
...
async def _mix_elixir(bot: Bot, event: GroupMessageEvent, mode : str = EventPlainText()):
    ...
    if matched == None:
        ...
        if XiuConfig().img:
            ...
            await mix_elixir.reject(prompt=f"@{event.sender.nickname}" + msg)
    else:
